...
const UpdateSupplier = ({opened = false, close} : UpdateSupplierTypes) => {
    const {supplier_id} = useSelector(getSuppliersState);
    const {setSupplierIdUpdate} = useActions();

    const [updateUser] = useUpdateUserMutation()

    const {data: supplier, refetch} = useGetUserFormQuery(supplier_id, {
        skip: supplier_id === 0,
    });
    ...
    useEffect(() => {
        if(supplier) {
            ...
            const strippedNumber = _.replace(supplier.phone, /^\+7/, '');
            ...
            setValue('position', supplier.position);
