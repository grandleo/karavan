...
const UpdateLogistic = ({opened = false, close} : UpdateLogisticTypes) => {
    const {logistic_id} = useSelector(getLogisticsState);
    const {setLogisticIdUpdate} = useActions();

    const [updateUser] = useUpdateUserMutation()

    const {data: client, refetch} = useGetUserFormQuery(logistic_id, {
        skip: logistic_id === 0,
    });
